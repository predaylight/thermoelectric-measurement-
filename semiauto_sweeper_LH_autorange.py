...
path='C:\\Users\\vcostanz\\Desktop\\Linghui\\P3_A\\sample4\\'
...
temperature_sweep_time=10 ### Time for temperature sweeping between impedance spect (min)
...
while True:
    ...
    if i%2==0:
        ...
        for j in range(len(frequency_end)):
            print('frequency', frequency_end[j],' to ',frequency_start[j],', num of pt ', npoint[j])
            lockIn.sweeperSet(frequency_start[j], frequency_end[j], npoint[j], scan, accuracy=accuracy)
            lockIn.setFrequency(frequency_start[j])
            autorange_current(deviceID,lockIn)
            time.sleep(5)
            print('autorange')
            lockIn.sweeperStart()
    ...
    if i!=0 :
        diff = np.abs(np.mean((reI_pre - np.flip(reI_now)) / reI_pre))
        reI_pre = reI_now
        print('\n\n\n difference is ',diff,'\n\n\n')        #
        if diff<0.05:
            temp.disableOutput()
